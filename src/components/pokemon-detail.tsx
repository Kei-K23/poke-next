...
export default function PokemonDetail({ id }: PokemonDetailProps) {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-8 flex gap-20 w-full">
        <div className="text-white w-full space-y-8">
          <h3 className="text-lg md:text-3xl lg:text-5xl capitalize font-bold">
          ...
          <div className="w-full flex justify-between">
            ...
            <div>
              <h3 className="text-xl md:text-2xl font-semibold mb-2">Types</h3>
              <div className="flex items-center gap-3">
                {pokemon.types.map((t) => (
                  <div
                    key={t.type.name}
          ...
          <div className="space-y-4">
            {pokemon.stats.map((stat, index) => (
              <div key={`${stat.stat}-${stat.base_stat}-${index}`}>
                <div className="flex items-center gap-x-3 text-xl font-semibold mb-2">
          ...
          <div className="text-black flex gap-4">
            <div className="neo-brutalism-white p-4 font-semibold">
              <p className="flex items-center gap-x-1 mb-2">
                <Ruler />
                HEIGHT
              </p>
              <span className="text-xl">{pokemon.height} M</span>
            </div>
            <div className="neo-brutalism-white p-4 font-semibold">
              <p className="flex items-center gap-x-1 mb-2">
                <Weight /> WEIGHT
              </p>
              <span className="text-xl">{pokemon.weight} KG</span>
            </div>
            <div className="neo-brutalism-white p-4 font-semibold">
              <p className="flex items-center gap-x-1 mb-2">
                <Activity /> BASE XP
              </p>
              <span className="text-xl">{pokemon.base_experience} XP</span>
            </div>
          </div>
        </div>
        <div className="relative w-full flex items-center justify-center">
          ...
          <Image
            ...
            width={700}
            height={700}
            className="object-contain absolute top-1/2 -translate-y-1/2"
          />
        </div>
      </div>
    </div>
